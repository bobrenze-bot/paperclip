...
function buildWakeText(
  ...
  claimedApiKeyPath = claimedApiKeyPath ?? "~/.openclaw/workspace/paperclip-claimed-api-key.json";
  const orderedKeys = [
    "PAPERCLIP_RUN_ID",
    "PAPERCLIP_AGENT_ID",
  ...
  const lines = [
    ...
    "   - GET /api/companies/$PAPERCLIP_COMPANY_ID/issues?assigneeAgentId=$PAPERCLIP_AGENT_ID&status=backlog,todo,in_progress,in_review,blocked",
    "   - Pick in_progress first, then in_review when you were woken by a comment, then todo, then backlog, then blocked, then execute step 3.",
    "",
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  // NOTE: agentParams.paperclip was removed — OpenClaw rejects unknown root properties
  // (additionalProperties: false in AgentParamsSchema). All Paperclip context is already
  // embedded in `message` via wakeText (PAPERCLIP_RUN_ID, PAPERCLIP_AGENT_ID, etc.).
  // paperclipPayload is still built above for potential future use.
