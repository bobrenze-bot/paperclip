      ...
      for (const row of due) {
        ...
        if (row.routine.catchUpPolicy === "enqueue_missed_with_cap") {
          ...
          while (cursor && cursor <= now) {
            totalCount += 1;
            if (withInCapCount < MAX_CATCH_UP_RUNS) {
              withInCapCount += 1;
            }
          ...
          if (totalCount > MAX_CATCH_UP_RUNS) {
            const droppedCount = totalCount - MAX_CATCH_UP_RUNS;
            await db
              .insert(routineCatchUpBreaches)
              .values({
                companyId: row.routine.companyId,
                routineId: row.routine.id,
                triggerId: row.trigger.id,
                missedCount: withInCapCount,
                capValue: MAX_CATCH_UP_RUNS,
                droppedCount,
              })
              .catch((err) => {
                logger.warn(
                  { err, routineId: row.routine.id, triggerId: row.trigger.id, totalCount, droppedCount },
                  "failed to record catch-up cap breach",
                );
              });

            if (row.routine.parentIssueId) {
              await issueSvc.addComment(row.routine.parentIssueId, `🚨 **Catch-Up Cap Breach Detected**

Routine: ${row.routine.title} (${row.routine.id})
Total missed runs: ${totalCount}
Captured (within cap): ${withInCapCount}
Dropped (exceeded cap): ${droppedCount}
Trigger: ${row.trigger.id} (nextRunAt: ${row.trigger.nextRunAt?.toISOString()})

The routine has exceeded the maximum catch-up runs cap and ${droppedCount} event(s) were silently dropped. This routine needs attention to prevent further event loss.

- **Auto-generated by:** routines.ts catch-up policy check
- **Timestamp:** ${new Date().toISOString()}
- **Routine ID:** ${row.routine.id}
- **Trigger ID:** ${row.trigger.id}
- **Total Missed:** ${totalCount}
- **Captured:** ${withInCapCount}
- **Dropped:** ${droppedCount}
- **Cap Value:** ${MAX_CATCH_UP_RUNS}
`, {
                runId: "system:routines:syncTriggersWithMissedRuns",
              });
              logger.info(`breach_alert: Posted comment for routine breach routineId=${row.routine.id} issueId=${row.routine.parentIssueId}`);
            }
          }
        }
